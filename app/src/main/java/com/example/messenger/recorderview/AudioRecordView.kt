package com.example.messenger.recorderview
...
import com.example.messenger.databinding.ViewAudioRecordBinding
import com.example.messenger.databinding.ViewSlidePanelBinding
...
class AudioRecordView : FrameLayout {
    ...
    constructor(context: Context, attrs: AttributeSet?, defStyleAttr: Int) : super(context, attrs, defStyleAttr) {
        ...
        context.withStyledAttributes(attrs, R.styleable.AudioRecordView) {
            if (hasValue(R.styleable.AudioRecordView_mic_icon)) {
                micIcon = getResourceId(R.styleable.AudioRecordView_mic_icon, 0)
            }
            if (hasValue(R.styleable.AudioRecordView_mic_active_icon)) {
                micActiveIcon = getResourceId(R.styleable.AudioRecordView_mic_active_icon, 0)
            }
            if (hasValue(R.styleable.AudioRecordView_mic_hint_enable)) {
                micHintEnable = getBoolean(R.styleable.AudioRecordView_mic_hint_enable, true)
            }
            if (hasValue(R.styleable.AudioRecordView_mic_hint_text)) {
                getString(R.styleable.AudioRecordView_mic_hint_text)?.let {
                    micHintText = it
                }
            }
            if (hasValue(R.styleable.AudioRecordView_mic_hint_color)) {
                micHintColor = getColor(R.styleable.AudioRecordView_mic_hint_color, 0)
            }
            if (hasValue(R.styleable.AudioRecordView_mic_hint_bg)) {
                micHintBg = getResourceId(R.styleable.AudioRecordView_mic_hint_bg, 0)
            }
            if (hasValue(R.styleable.AudioRecordView_circle_color)) {
                circleColor = getColor(R.styleable.AudioRecordView_circle_color, 0)
            }
            if (hasValue(R.styleable.AudioRecordView_cancel_icon_color)) {
                cancelIconColor = getColor(R.styleable.AudioRecordView_cancel_icon_color, 0)
            }
            if (hasValue(R.styleable.AudioRecordView_blink_color)) {
                blinkColor = getColor(R.styleable.AudioRecordView_blink_color, 0)
            }
            if (hasValue(R.styleable.AudioRecordView_slide_cancel_text)) {
                getString(R.styleable.AudioRecordView_slide_cancel_text)?.let {
                    slideCancelText = it
                }
            }
            if (hasValue(R.styleable.AudioRecordView_cancel_text)) {
                getString(R.styleable.AudioRecordView_cancel_text)?.let {
                    cancelText = it
                }
            }
            if (hasValue(R.styleable.AudioRecordView_vibration_enable)) {
                vibrationEnable = getBoolean(R.styleable.AudioRecordView_vibration_enable, true)
            }
    ...
    private fun updateRecordCircleAndSendIcon() {
        if (isRecording) {
            binding.recordCircle.visibility = VISIBLE
            ...
            ObjectAnimator.ofFloat(binding.recordCircle, "scale", 0f).apply {
                ...
                addListener(
                    onEnd = {
                        binding.recordCircle.visibility = GONE
    ...
    private fun clickSend() {
        if (micHintEnable && binding.recordTipTv.isInvisible) {
    ...
    private val hideRecordTipRunnable = Runnable {
        if (binding.recordTipTv.isVisible) {
